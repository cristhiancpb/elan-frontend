...
function Ventas() {
  ...
  function generarFactura() {
    ...
    items.forEach(it => {
      const idx = actualizados.findIndex(p => p.nombre === it.nombre)
      if (idx >= 0) actualizados[idx].stock -= it.cantidad
    })
    localStorage.setItem('productos', JSON.stringify(actualizados))
    alert('✅ Factura generada. Stock actualizado.')
    setItems([])
  }
          ...
          <div style={{ background: '#161b22', border: '1px solid #30363d', borderRadius: '12px', padding: '28px' }}>
            <div style={{ marginBottom: '16px' }}>
              ...
              <select value={selIndex} onChange={e => setSelIndex(parseInt(e.target.value))} style={inputStyle}>
                {productos.map((p, i) => (
                  <option key={i} value={i}>{p.nombre} — ${p.precio.toLocaleString('es-CO')}</option>
                ))}
              </select>
            </div>
